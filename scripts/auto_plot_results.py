...
parser.add_argument("--n-files", type=int, default=5) # set to 150 or 300 for more proper eval
    ...
    while True:
        ...
        for f in files:
            ...
            if os.path.exists(current_folder_path):# and not args.redo:
                ...
                file = datasets[dataset]["eval"] + "pf_tree_{4000..}" + str(4000 + args.n_files) + "}"
                cmd = eval_cmd.format(model_prefix=current_folder_path, gpu=gpu, ckpt_file=ckpt_file, file=file)
                cmdargs = cmd.split()

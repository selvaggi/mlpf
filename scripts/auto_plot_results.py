...
os.environ["PYTHONPATH"] = os.environ.get("PYTHONPATH", "") + ":/afs/cern.ch/work/g/gkrzmanc/mlpf_2024"
...
while True:
    ...
    for f in files:
        ...
        if os.path.exists(plots_stdout):
            ...
            with open(plots_stderr, "w") as f:
                if err is not None:
                    ...
                    f.write("None")
    time.sleep(pause)
    if time() - os.path.getmtime(os.path.join(args.path, files[-1])) > timeout:

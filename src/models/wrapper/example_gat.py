...
class GATWrapper(torch.nn.Module):
    def __init__(self, dev, **kwargs) -> None:
        ...
        self.mod2 = GATConv(in_channels=128, out_channels=128, heads=3, concat=False, **kwargs).to(dev)
        self.mod3 = GATConv(in_channels=128, out_channels=128, heads=3, concat=False, **kwargs).to(dev)

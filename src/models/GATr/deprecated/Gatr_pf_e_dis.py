...
class ExampleWrapper(L.LightningModule):
    ...
    def __init__(
        ...
        self.dist = nn.Linear(5, 1)
        self.dist_softsign = nn.Softsign()
    ...
    def forward(self, g, y, step_count, eval=""):
        ...
        dist = self.dist(torch.cat((x_scalar, x_point), dim=1))
        dist = torch.abs(self.dist_softsign(dist) * 10)
        if self.args.tracks:
        ...
        x = torch.cat((x_cluster_coord, beta.view(-1, 1), dist.view(-1, 1)), dim=1)

...
def create_block_diagonal(matrices):
    """
    Create a block diagonal matrix from a list of 2D matrices.

    Args:
        matrices (list of torch.Tensor): List of 2D tensors (matrices) to place on the block diagonal.
...
class ExampleWrapper(L.LightningModule):
    ...
    def build_attention_mask(self, g):
        ...
        attention_mask : xformers.ops.fmha.BlockDiagonalMask
            ...
            token.
        """
        device = g.ndata["hit_type"].device
        block_diag = g.adj().to_dense().to(device)
        return block_diag 
    ...
    def configure_optimizers(self):
        ...
        #     eta_min=1e-6,
        # )
        scheduler = CosineAnnealingThenFixedScheduler(optimizer,T_max=int(7700*5), fixed_lr=1e-6 ) #10000
        return {
            "optimizer": optimizer,
            "lr_scheduler": {

...
from src.gatr_v111.interface import embed_point, extract_scalar, extract_point, embed_scalar
...
class ExampleWrapper(L.LightningModule):
    ...
    def forward(self, inputs):
        pos_hits_xyz = inputs[:, 0:3]
        inputs_scalar =  inputs[:, 3].view(-1, 1)
        e_hits = inputs[:, 4].view(-1, 1)
        p_hits = inputs[:, 5].view(-1, 1)
        pos_hits_xyz = self.ScaledGooeyBatchNorm2_1(pos_hits_xyz)
        # inputs = inputs.unsqueeze(0)
        embedded_inputs = embed_point(pos_hits_xyz) + embed_scalar(inputs_scalar)
        embedded_inputs = embedded_inputs.unsqueeze(
            -2
        )  # (batch_size*num_points, 1, 16)
 
        scalars = torch.zeros((inputs.shape[0], 1))
        scalars = torch.cat((e_hits, p_hits), dim=1)  

import numpy as np
import torch
from src.models.point_transformer.helper import (
    index_points,
    square_distance,
    TransitionDown,
    TransitionUp,
...
class PointTransformerBlock(nn.Module):
    ...
    def forward(self, x, pos):
        ...
        attn = torch.softmax(attn / np.sqrt(k.size(-1)), dim=-2)  # b x n x k x f
...
class PointTransformer(nn.Module):
    def __init__(
        ...
        self.ptb = PointTransformerBlock(hidden_dim, n_neighbors, transformer_dim)
...
class PointTransformerOC(nn.Module):
    def __init__(
        self,
        args,
        dev,
    ...
    def forward(self, g, step_count):  # x, cat_vec=None):

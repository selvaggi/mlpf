...
def find_pandora_pfo_and_cluster_of_hit(
    ...
    for index_pfo, pfo in enumerate(pfo_collection):
        ...
        if pandora_cluster_index >= 0:
            break
    return pandora_cluster_index, cluster_energy_found, pfo_energy_found
...
def store_tracks(
    event, debug, dic, genpart_indexes, n_hit, number_of_hist_with_no_genlinks
):
    ## track stuff
    ...
    for j, track in enumerate(event.get(track_coll)):
        ...
        # print(dir(trackstate))
        # trackstate = event.get(trackstates)[4 * j]
        # print(dir(trackstate))
...
def store_calo_hits_pandora(
    ...
    for k, calohit_coll in enumerate(calohit_collections):
        ...
        for j, calohit in enumerate(event.get(calohit_coll)):
            ...
            if ngen > 0:
                genlink = link_vector[0]
            pandora_cluster = find_pandora_cluster_of_hit(
                j, hit_collection, pandora_clusters_event
            )
            check_pandora_pfos(pandora_pfos_event)
            break
            if len(gen_indices) > 0:
                dic["hit_genlink0"].push_back(gen_indices[0])
            if len(gen_indices) > 1:

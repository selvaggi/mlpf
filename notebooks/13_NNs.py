                ...
                for i in (pbar := tqdm.tqdm(range(0, num_batches))):
                    ...
                    if i % 100 == 0 and self.model.wandb_log_name is not None:
                        ...
                        if pid is not None:
                            ...
                            wandb.log({"confusion_matrix" + self.model.wandb_log_name: wandb.sklearn.plot_confusion_matrix(pidtrue_idx.cpu().numpy(), pidpred_idx.cpu().numpy(), class_names=class_names)})
                    if i < 3000:

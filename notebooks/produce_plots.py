...
## used for paper:
## "/eos/user/m/mgarciam/datasets_mlpf/models_trained/all_energies_karolina/evaluation/analysis/out.bin.gz"

with gzip.open(
    "/eos/user/m/mgarciam/datasets_mlpf/models_trained/logs_1015_1911/pandora/analysis/out.bin.gz",
    "rb",
) as f:
    data = pickle.load(f)
...
dict_1 = obtain_metrics(sd, matched, ms)

dic2 = False
if dic2:

    with gzip.open(
        "/eos/user/m/mgarciam/datasets_mlpf/models_trained/RS_0/large_eval/analysis/out.bin.gz",
        "rb",
    ) as f:
        data = pickle.load(f)
    sd = data["showers_dataframe"]
    matched = sd.dropna()
    ms = data["matched_showers"]
    dict_2 = obtain_metrics(sd, matched, ms)

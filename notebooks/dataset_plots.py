...
class Args:
    def __init__(self):
        ...
        self.data_config = 'C:/Users/Asus/Desktop/mlpf-main/config_files/config_2_knn_graph_TRACKS.yaml'
...
tree = ET.parse('C:/Users/Asus/Desktop/cern project/detector-simulation-master/geometries/CLIC_o3_v14/CLIC_o3_v14.xml')
root = tree.getroot()
...
for i in range(number_of_plots):
    ...
    symbol = ms), name = "particle", text = e_particle))
    #fig plot hits with size of particle energy color particle_id, shape hit_type
    fig.add_trace(go.Scatter3d(x= pos_particles[:,0], y= pos_particles[:,1], z= pos_particles[:,2], mode = "markers",
                                marker = dict(size = e_particle, color = particle_id, symbol = ms), name = "hits", text  = e_particle , ))
    
    #plot H_cal and E_cal 
    fig2 = go.Figure(data=[
    go.Surface(z=H_x_outer, y=H_y_outer, x=H_z, opacity=0.3,colorscale= h_colorscale,name = "HCAL"),
    go.Surface(z=H_x_inner, y=H_y_inner, x=H_z, opacity=0.3,colorscale = h_colorscale, name = "HCAL"),
    go.Surface(z=E_x_outer, y=E_y_outer, x=E_z, opacity=0.2, colorscale= e_colorscale,name = "ECAL"),
    go.Surface(z=E_x_inner, y=E_y_inner, x=E_z, opacity=0.2,colorscale= e_colorscale, name = "ECAL")
])
    fig2.update_layout(
        scene=dict( 
            aspectmode="manual",
    ...
    fig.update_layout(
        updatemenus = [
            dict(buttons = list([dict(label = "Plots", method = "update",args = [{"visible": [True, True,True, True, True,True]},{"title": "Plots"}]),
                    dict(label = "particle_id",method = "update", args = [{"visible": [True ,False, False, False, False, False]},{"title": "particles"}]),
                    dict (label = "hits",method = "update",args = [{"visible" : [False , True,False, False, False, False]},{"title": "hits"}]),
                    dict( label = "E_cal", method = 'update', args = [{"visible": [True , True,True, True,False, False]}, {"title":"E_cal"}]),
                    dict( label = "H_cal", method = 'update', args = [{"visible": [True , True,False, False,True, True]}, {"title":"H_cal"}])
                    ])
